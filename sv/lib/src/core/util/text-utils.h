...
bool SplitStringToIntegers(const std::string &full,
                           ...
                           bool omit_empty_strings,  // typically false [but
                                                     ...
                                                     // if "delim" is spaces].
                           std::vector<I> *out) {
  IDEC_ASSERT(out != NULL);
  IDEC_ASSERT_IS_INTEGER_TYPE(I);
  if ( *(full.c_str()) == '\0') {
    out->clear();
    return true;
  }
...
bool ConvertStringToInteger(const std::string &str,
                            Int *out) {
  //IDEC_ASSERT_IS_INTEGER_TYPE(Int);
  const char *this_str = str.c_str();
  char *end = NULL;
  errno = 0;
  long long int i = IDEC_STRTOLL(this_str, &end);
  if (end != this_str)
    while (isspace(*end)) end++;
  if (end == this_str || *end != '\0' || errno != 0)
    return false;
  Int iInt = static_cast<Int>(i);
  if (static_cast<long long int>(iInt) != i || (i<0
