...
void ParseOptions::RegisterCommon(const std::string &name, T *ptr,
  ...
  IDEC_ASSERT(ptr != NULL);
  std::string idx = name;
  NormalizeArgName(&idx);
  if (doc_map_.find(idx) != doc_map_.end())
    IDEC_WARNING << "Registering option twice, ignoring second time: " << name;
...
void ParseOptions::DisableOption(const std::string &name) {
  if (argv_ != NULL)
    IDEC_ERROR << "DisableOption must not be called after calling Read().";
  if (doc_map_.erase(name) == 0)
    IDEC_ERROR << "Option " << name
...
std::string ParseOptions::GetArg(int i) const {
  if (i < 1 || i > static_cast<int>(positional_args_.size()))
    IDEC_ERROR << "ParseOptions::GetArg, invalid index " << i;  // code error
  // so use IDEC_ERROR
...
static bool MustBeQuoted(const std::string &str, ShellType st) {
  ...
  IDEC_ASSERT(st == kBash);  // Nothing else supported right now.
  ...
  if (*c == '\0') return true;  // Must quote empty string
    ...
    IDEC_ASSERT(!strchr(ok_chars[kBash], ' ')); // Just want to make sure that
  ...
  for (i = 1; i < argc; i++) {
    if (std::strncmp(argv[i], "--", 2) == 0) {
      ...
      if (!SetOption(key, value, has_equal_sign)) {
        PrintUsage(true);
        IDEC_ERROR << "Invalid option " << argv[i];
      }
    } else {
      break;
    }
...
void ParseOptions::PrintConfig(std::ostream &os) {
  ...
  for (it = doc_map_.begin(); it != doc_map_.end(); ++it) {
    ...
    if (bool_map_.end() != bool_map_.find(key)) {
      ...
      IDEC_ERROR << "PrintConfig: unrecognized option " << key << "[code error]";
    }
    os << '\n';
  }
  os << '\n';
...
void ParseOptions::ReadConfigFile(const std::string &filename) {
  ...
  if (!is.good()) {
    IDEC_ERROR << "Cannot open config file: " << filename;
  }
  ...
  while (std::getline(is, line)) {
    ...
    if (line.substr(0, 2) != "--") {
      IDEC_ERROR << "Reading config file " << filename
    ...
    if (!SetOption(key, value, has_equal_sign)) {
      //PrintUsage(true);
      //IDEC_ERROR << "Invalid option " << line << " in config file " << filename;
    }
  }
}
...
void ParseOptions::SplitLongArg(std::string in,
                                ...
                                std::string *value,
                                bool *has_equal_sign) {
  IDEC_ASSERT(in.substr(0, 2) == "--");  // precondition.
  ...
  if (pos == std::string::npos) {  // we allow --option for bools
    ...
    *has_equal_sign = false;
    ...
    PrintUsage(true);
    IDEC_ERROR << "Invalid option (no key): " << in;
...
bool ParseOptions::ToBool(std::string str) {
  ...
  PrintUsage(true);
  IDEC_ERROR << "Invalid format for boolean argument [expected true or false]: "
             << str;
...
int32 ParseOptions::ToInt(std::string str) {
  ...
  if (str.c_str() == end_pos) {
    PrintUsage(true);
    IDEC_ERROR << "Invalid integer option \"" << str << "\"";
  }
  return ret;
}
...
float ParseOptions::ToFloat(std::string str) {
  ...
  float ret = (float)std::strtod(str.c_str(), &end_pos);
  if (str.c_str() == end_pos) {
    PrintUsage(true);
    IDEC_ERROR << "Invalid floating-point option \"" << str << "\"";
  }
  return ret;
}
