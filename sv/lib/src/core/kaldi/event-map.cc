...
SplitEventMap *SplitEventMap::Read(std::istream &is, bool binary) {
  ...
  if (yes == NULL
      || no == NULL) KALDI_ERR << "SplitEventMap::Read, NULL pointers.";
...
void EventMap::Check(const
                     std::vector<std::pair<EventKeyType, EventValueType> > &event) {
...
TableEventMap::TableEventMap(EventKeyType key,
                             const std::map<EventValueType, EventMap *> &map_in): key_(key) {
  if (map_in.size() == 0)
    return;  // empty table.
  else {
...
TableEventMap::TableEventMap(EventKeyType key,
                             const std::map<EventValueType, EventAnswerType> &map_in): key_(key) {
  if (map_in.size() == 0)
    return;  // empty table.
  else {
    EventValueType highest_val = map_in.rbegin()->first;
    table_.resize(highest_val+1, NULL);
    std::map<EventValueType, EventAnswerType>::const_iterator iter =
      map_in.begin(), end = map_in.end();
    for (; iter != end; ++iter) {
      KALDI_ASSERT(iter->first >= 0 && iter->first <= highest_val);
