...
#include <map>
#include <algorithm>
#include "kaldi-idec-common.h"
#include "kaldi/stl-utils.h"
#include "kaldi/const-integer-set.h"
...
std::string EventTypeToString(const EventType
                              &evec);  // so we can print events out in error messages.
...
class EventMap {
 public:
  static void Check(const EventType
                    &event);  // will crash if not sorted and unique on key.
  ...
  virtual EventAnswerType MaxResult()
  const {  // child classes may override this for efficiency; here is basic version.
...
class TableEventMap: public EventMap {
 public:
  ...
  virtual EventMap *Copy(const std::vector<EventMap *> &new_leaves) const {
    ...
    for (size_t i = 0; i<table_.size();
         i++) if (table_[i]) new_table_[i]=table_[i]->Copy(new_leaves);
...
class SplitEventMap: public EventMap {  // A decision tree [non-leaf] node.
 public:
  ...
  SplitEventMap(EventKeyType key, const std::vector<EventValueType> &yes_set,
                EventMap *yes, EventMap *no): key_(key), yes_set_(yes_set), yes_(yes),
    no_(no) {
